...
assert.match(html, /retail:\s*\[[^\]]*'DG'[^\]]*'DLTR'[^\]]*'ROST'[^\]]*'TJX'/);
...
assert.match(html, /mirai-proxy\.jay-nishiguchi\.workers\.dev\/\?url=/);
assert.doesNotMatch(html, /target => `https:\/\/api\.allorigins\.win/);
assert.doesNotMatch(html, /target => `https:\/\/corsproxy\.io/);
